...
my_test_provisioning_uuid = '32913f8db6a6bc57e8352df9744d96c23a7241ad'
...
def setup_commands():
	...
	for cmd in ['DeviceLock', 'ProfileList', 'Restrictions', 
		'CertificateList', 'InstalledApplicationList', 'ProvisioningProfileList'] :
	...
	ret_list[cmd] = dict(
		Command = dict(
			...
			Queries = [
				...
				'PasscodeCompliant', 'PasscodeComplaintWithProfiles',
			]
		)
	)
	...
	ret_list['DeviceInformation'] = dict(
		Command = dict(
			...
			Queries = [
				'AvailableDeviceCapacity', 'BluetoothMAC', 'BuildVersion', 'CarrierSettingsVersion',
				'CurrentCarrierNetwork', 'CurrentMCC', 'CurrentMNC', 'DataRoamingEnabled',
				'DeviceCapacity',  'DeviceName', 'ICCID', 'IMEI', 'IsRoaming',
				'Model', 'ModelName', 'ModemFirmwareVersion', 'OSVersion', 'PhoneNumber',
				'Product', 'ProductName', 'SIMCarrierNetwork', 'SIMMCC', 'SIMMNC', 'SerialNumber',
				'UDID', 'WiFiMAC', 'UDID'
			]
		)
	)
...
class enroll_profile:
    def GET(self):
        ...
        if 'test.mobileconfig' in os.listdir('.'):
            ...
            web.header('Content-Disposition', 'attachment;filename="test.mobileconfig"')
            return open('test.mobileconfig', "rb").read()
        else:
            raise web.notfound()
...
# 		import pdb; pdb.set_trace()
		...
		print('my_DeviceToken ' + my_DeviceToken)
		print('my_PushMagic token ' + my_PushMagic)
		
		wrapper = APNSNotificationWrapper('PlainCert.pem',sandbox=False)
...
class do_mdm:
	...
	def PUT(self):
		...
		elif pl.get('Status') == 'Acknowedged':
			rd = dict()
		else:
			rd = dict()
		...
		if q:
			redact_list = ('UUID', 'BluetoothMAC', 'SerialNumber', 'WiFiMAC',
...
def home_page():
	...
	for key in mdm_commands:
		print("---------" + current_command['Command']['RequestType'] + "-----------")
	...
	<select name="command"><option value=''>Select command</option>
	%s
	</select>
...
def do_TokenUpdate(pl):
	...
	myUnlockToken = pl['UnlockToken'].data
...
my_DeviceToken = '%s'
myUnlockToken = '%s'
""" % (my_PushMagic, repr(my_DeviceToken), repr(myUnlockToken))
